...
| LLVM (installed) | 14–17 tested | apt package works |
| C++ compiler | C++17 | prefer `g++`; `clang++` may need `libstdc++-dev` |
...
| `build/lib/Kotoamatsukami.so` | Obfuscation plugin for `-fpass-plugin=` |
| `build/lib/learn_llvm_pass.so` | Simple pass plugin |
| `build/lib/learn_llvm_deobf.so` | Deobfuscation plugin |
| `build/lib/libhello_pass_lib.a` | Static lib linked by tests |
